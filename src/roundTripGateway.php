...
class roundTripGateway
{
    ...
    public function getRoundTrip($departure_airport, $arrival_airport, $departure_date, $return_date):array
    {
        ...
        $sql = "SELECT  f1.flight_number AS firstFlightNumber, f2.flight_number AS secondFlightNumber, f1.airline_code AS firstFlightAC, f2.airline_code AS secondFlightAC, al.name AS airline_name, f1.departure_time AS departure_datetime,f1.arrival_time, f1.departure_airport_code AS departure_airport_code, f2.departure_time AS return_departure_datetime, f2.arrival_time AS return_arrival_datetime, f1.arrival_airport_code AS arrival_airport_code, f1.price + f2.price AS total_price,f1.price AS firstflight_price, f2.price AS secondflight_price                
        FROM flights f1
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC))
        {
            $data = [
                'trips' =>[
                    ...
                    'flights' =>[
                        [
                            'airline'=>$row['firstFlightAC'],
                            'number' =>$row['firstFlightNumber'],
                            'departure_airport' =>$row['departure_airport_code'],
                            'departure_datetime' =>$row['departure_datetime'],
                            'arrival_airport' =>$row['arrival_airport_code'],
                            'arrival_datetime' =>$row['arrival_time'],
                            'price' =>$row['firstflight_price']
                        ],
                        [
                            'airline'=>$row['secondFlightAC'],
                            'number' =>$row['secondFlightNumber'],
                            'departure_airport' =>$row['arrival_airport_code'],
                            'departure_datetime' =>$row['return_departure_datetime'],
                            'arrival_airport' =>$row['departure_airport_code'],
                            'arrival_datetime' =>$row['return_arrival_datetime'],
                            'price' =>$row['secondflight_price']
                        ]
